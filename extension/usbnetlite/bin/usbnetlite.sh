#!/bin/sh
#
# KPM USBNetwork actions helper script
#
...
## Start USBNetwork when it is not already running
start_usbnet()
{
	if check_is_plugged_in ; then
		return 1
	...
	kh_msg "Start USBNetwork" I a
	${KH_HACK_BINDIR}/usbnetwork usbnet
}

## Stop USBNetwork when it is running
stop_usbnet()
{
	if check_is_plugged_in ; then
		return 1
	...
	kh_msg "Stop USBNetwork" I a
	${KH_HACK_BINDIR}/usbnetwork usbms
}
...
restore_config()
{
	...
	if [ -f "${KH_HACK_BASEDIR}/etc/config.default" ] ; then
		kh_msg "Restore default config file" I a
		cp -f "${KH_HACK_BASEDIR}/etc/config.default" "${KH_HACK_BASEDIR}/etc/config"
	else
